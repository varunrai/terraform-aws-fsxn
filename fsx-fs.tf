...
resource "aws_fsx_ontap_file_system" "fsx_ontap_fs" {
  ...
  deployment_type     = var.fsxn_deployment_type_multiaz ? "MULTI_AZ_1" : "SINGLE_AZ_1"
  subnet_ids          = var.fsxn_subnet_ids
  preferred_subnet_id = var.fsxn_deployment_type_multiaz ? var.fsxn_preferred_subnet_id : var.fsxn_subnet_ids[0]
